...
control.status.rn <- function(data,
  ...
  data %>%
    ...
    if(combine) {
      data %>%
        mutate(New.control.status.recode =
                 case_when(factor2numeric(New.control.status.recode) == 1 |
                             factor2numeric(New.control.status.recode) %in% c(2,12) ~ "Owner-occupied"
                           , factor2numeric(New.control.status.recode) == 80 ~ "Market rate"
                           , factor2numeric(New.control.status.recode) == 30 |
                             factor2numeric(New.control.status.recode) == 31 |
                             factor2numeric(New.control.status.recode) == 90 ~ "Controlled/Stabilized"
                           , factor2numeric(New.control.status.recode) == 5 |
                             factor2numeric(New.control.status.recode) == 85 |
                             factor2numeric(New.control.status.recode) == 86 |
                             factor2numeric(New.control.status.recode) == 21 ~ "HUD/NYCHA/Mitchell Lama"
                           , factor2numeric(New.control.status.recode) == 95 ~ "In Rem"
                 ))
    }
}
